...
window.run = async () => {
  ...
  const rt = new QwenWGPU(dev, cfg, FUSED); await rt.build('/model');
  ...
  const runDecode = async (flags, label) => {
    ...
    const logits = await readLogits(); const gen = await decodeN(ids.length, 8);
  ...
  const runPrefill = async (flags, label, prompt) => {
    ...
    const dispatches = rt.lastDispatchCount;
    const logits = await readLogits(); const gen = await decodeN(prompt.length, 8);
  ...
  console.log(`VWG ${decodeOk ? 'PASS' : 'FAIL'} old decode vs fused decode logitΔ=${maxAbsDiff(oldDec.logits, fusedDec.logits).toFixed(6)}`);

  const tile = (L) => { const out = []; while (out.length < L) out.push(ids[out.length % ids.length]); return out; };
  let prefillOk = true;
  for (const L of [16, 257, 512]) {
    const prompt = tile(L);
    const row = await runPrefill({ ...BASE, prefillAttention: 'row' }, `row L=${L}`, prompt);
    const block = await runPrefill({ ...FUSED, prefillAttention: 'block', prefillChunkSize: 0 }, `block L=${L}`, prompt);
    const chunk = await runPrefill({ ...FUSED, prefillAttention: 'block', prefillChunkSize: 64 }, `chunk64 L=${L}`, prompt);
    const ok = sameGen(row, block) && sameGen(row, chunk) && argmax(row.logits) === argmax(block.logits) && argmax(row.logits) === argmax(chunk.logits);
    prefillOk = prefillOk && ok;
    console.log(`VWG ${ok ? 'PASS' : 'FAIL'} prefill row/block/chunk L=${L} blockΔ=${maxAbsDiff(row.logits, block.logits).toFixed(3)} chunkΔ=${maxAbsDiff(row.logits, chunk.logits).toFixed(3)}`);
  }

  try {
    const mk = async (path, name) => { const res = await fetch(path); if (!res.ok) throw new Error(`${path} ${res.status}`); const buf = await res.arrayBuffer(); return { name, async text() { return new TextDecoder().decode(buf); }, async arrayBuffer() { return buf; } }; };
    const adapterFiles = [await mk('/adapters_sel/adapter_config.json', 'adapter_config.json'), await mk('/adapters_sel/adapters.safetensors', 'adapters.safetensors')];
    ...
    console.log(`VWG ${loraOk ? 'PASS' : 'FAIL'} old LoRA decode vs fused LoRA decode logitΔ=${maxAbsDiff(oldLora.logits, fusedLora.logits).toFixed(6)}`);
...
window.addEventListener('DOMContentLoaded', () => window.run().catch(e => console.log('VWG ERROR ' + e.message + ' | ' + (e.stack || '').slice(0, 400))));
